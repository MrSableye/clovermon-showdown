...
export class RandomTeams {
	...
	random10uTeam(): RandomTeamsTypes.RandomSet[] {
		return [{
			name: "Magikarp",
			species: "Magikarp",
			shiny: this.randomChance(1, 1024),
	...
	randomSpookyTeam() {
		const seed = this.prng.seed;
		const pokemon: RandomTeamsTypes.RandomSet[] = [];
		const clovermon: RandomTeamsTypes.RandomSet[] = [];
	...
	getPokemonPool(type: string, pokemon: RandomTeamsTypes.RandomSet[] = [], isMonotype = false, isCloveronly = false) {
		const exclude = pokemon.map(p => toID(p.species));
		const pokemonPool = [];
		for (const id in this.dex.data.FormatsData) {
			let species = this.dex.species.get(id);
			if (species.gen > this.gen || exclude.includes(species.id)) continue;
			if (isCloveronly && (species.num <= 69000 || species.num > 69386)) continue;
			if (isMonotype) {
