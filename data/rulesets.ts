...
export const Formats: {[k: string]: FormatData} = {
	...
	fochunonly: {
		effectType: 'ValidatorRule',
		name: 'Fochun Only',
		desc: "Only allows Pok&eacute;mon, items, and move available in the Fochun and Ebin Isles (Clover)",
		onValidateSet(set) {
			const fochunMoves = [
				"pound",
				"karatechop",
			...
			if (species.num <= 69000 || species.num > 69386) {
				errors.push(`${species.baseSpecies} is not in the Fochun Pokédex.`);
			}

			if (!fochunItems.includes(this.toID(set.item))) {
				errors.push(`${set.name || set.species} has ${set.item}, which is unavailable in Fochun or the Ebin Isles.`);
			}
