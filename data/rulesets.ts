...
export const Formats: {[k: string]: FormatData} = {
	...
	fochunpokedex: {
		effectType: 'ValidatorRule',
		name: 'Fochun Pokedex',
		desc: "Only allows Pok&eacute;mon native to the Fochun and Ebin Isles (Clover)",
		onValidateSet(set, format) {
			const species = this.dex.getSpecies(set.species || set.name);
			if (species.num <= 69000 || species.num > 69386) {
				return [`${species.baseSpecies} is not in the Fochun Pokédex.`]
			}
		},
	},
	fochunitems: {
		effectType: 'ValidatorRule',
		name: 'Fochun Items',
		desc: "Only allows items available in Fochun and Ebin Isles (Clover)",
		onValidateSet(set, format) {
			const fochunItems = [
				"????????",
				"Master Ball",
			...
			if (!fochunItems.includes(this.toID(set.item))) {
				return [`${set.name || set.species} has ${set.item}, which is unavailable in Fochun or the Ebin Isles.`];
			}
		},
	}
};
