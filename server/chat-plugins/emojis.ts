...
export const commands: Chat.ChatCommands = {
	...
	emoji: {
		list() {
			this.runBroadcast();
			return this.sendReplyBox(`<b><u>Emojis <i>(try <code>:EMOJINAME:</code>)</i></b><br />${Object.entries(emojis).map(([emojiName, emojiUrl]) => createEmojiHtml(emojiName, emojiUrl)).join(' ')}`);
		},
		update: 'add',
		async add(target) {
			this.checkCan('emoji');
